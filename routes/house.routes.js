...
router.get("/my-houses", isAuthenticated, async (req, res, next) => {
  try {
    ...
    for (let i = 0; i < houses.length; i++) {
      ...
      if (houseID == userId) housesByAuthor.push(currentHouse);
    }
...
router.get("/:houseId", async (req, res, next) => {
  try {
    ...
    const allContacts = await Contact.find({ house: houseId }).populate(
      "author"
    );
  ...
  async (req, res, next) => {
    try {
      ...
      return res
        .status(200)
        .json({
          message: `House ${houseId} saved as favourite`,
...
router.post("/:houseId/removefav", isAuthenticated, async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json({
        message: `House ${houseId} removed from favourite list`,
        users: newUser.bookmarkList,
      });
  } catch (error) {
    next(error);
